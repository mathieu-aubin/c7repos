...
_installREPOS() {
	...
	enabled=1
	__EOF__
	echo -e "  - \033[32mRepository file for MariaDB 10.3 created\033[0;1m.\033[0m"; sleep 0.3 ;
...
_updateGRUB() {
    ...
    rm -f /boot/grub2/grubenv &>/dev/null ; ${HOME}/bin/update-grub ; echo ;
}
...
_installNODE() {
    ...
    case "${_NODENPM}" in
      ...
      *)
        ...
        echo -e "\033[1mCreating NPM bash completion file...\033[0m" ; sleep 0.2 ;
        ...
        echo -e "  - \033[32mNPM bash completion file created\033[0;1m.\033[0m" ; sleep 0.3 ;
        ;;
    esac
...
_installCOMMON() {
    ...
    [[ $? -ne 0 ]] && {
        echo -e "\033[1mCreating PIP bash completion file...\033[0m" ; sleep 0.2 ;
        # Upgrades pip to latest version
        pip install pip --upgrade &>/dev/null ;
        # Generates pip bash completion file
        ...
        echo -e "  - \033[32mPIP bash completion file created\033[0;1m.\033[0m" ; sleep 0.3 ;
    }
...
_createDOTFILES() {
    ...
    echo 'for DFs in $(ls -1p ${HOME}/.bash_* | \grep -Ev "history|logout|profile|back|/$"); do source ${DFs}; done' >> ${HOME}/.bashrc ;
    ...
    echo -e "# .bash_functions - Functions file added by an awesome script on $(date +"%F %R:%S")\n# Get it at ${BRAGURL}\n#" >> ${HOME}/.bash_functions ;
        ...
        while read _FSRC; do
            curl -skL ${REPOURL}/functions/${_FSRC} >> ${HOME}/.bash_functions ;

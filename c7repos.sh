...
_installREPOS() {
    ...
    curl -sL https://download.docker.com/linux/centos/docker-ce.repo >> /etc/yum.repos.d/docker-ce.repo && \
...
_createDOTFILES() {
    ...
    echo -e "\nshopt -s checkwinsize\nshopt -s histappend\nexport HISTIGNORE='&:exit:x:l:history:editenv:[h ]*:[ \\\t]*:?:??:ip6?tables -S:w -i:pwd:srcalias*:addalias*:srcexports*:addexport*:srcfunctions*:addfunction*:srccolors*:addcolors*:srcdot*:env:nginx -t:quit:popd*:ginx*:chownwww*:yum clean*:yum makecache*'\nexport HISTCONTROL=ignoreboth\nexport HISTSIZE=20000\nexport HISTFILESIZE=100000\nexport HISTTIMEFORMAT=\"%Y/%m/%d %T \"" >> ${HOME}/.bashrc ;
